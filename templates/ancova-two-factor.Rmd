...
pdat = tryCatch(
  remove_group_data(dat[!is.na(dat[["{{ iv1 }}"]]) & !is.na(dat[["{{ iv2 }}"]]),],
                         "{{ dv.pos }}", c("{{ iv1 }}","{{ iv2 }}")),
...
```{r}
if (!is.null(pdat) && length(unique(pdat[["{{ iv2 }}"]])) >= 2) {
  pwcs <- list()
...
saveRDS(
  ...
  file = '{{ rds.prefix }}-ancova-{{ gsub("[^a-zA-Z0-9]","",dv.pos) }}-from-{{ gsub("[^a-zA-Z0-9]","",dv.pre) }}-{{ gsub("[^a-zA-Z0-9]","",iv1) }}-{{ gsub("[^a-zA-Z0-9]","",iv2) }}.rds')
```
